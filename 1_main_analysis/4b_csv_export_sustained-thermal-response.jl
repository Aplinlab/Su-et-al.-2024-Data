...
for comparison in eachrow(df)
    ...
    # Overwrite WOI for laser cells and define bin size
    if stim_code == "L1"
        WOI2 = [1500, 4500]
        WOI1 = [-1000, 0]
...
CSV.write("r_analysis/full_results sustained-thermal-response $date.csv", outputTable)
println("CSV Exporting Complete")

if !isempty(error_log)
    now = Dates.now()
    log_file = open("../error_logs/main_analysis.txt", "a")
    println(log_file, "---CSV Exporting (sustained thermal response) $now---\n\n$error_log")
    close(log_file)
end